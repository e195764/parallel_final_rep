...
print("LinearSVCの正解率 = " + str(accuracy_score(y_test, predicted)))

# F値, F1 = 2 * (precision * recall) / (precision + recall)
print("LinearSVCのF値 = " + str(f1_score(y_test, predicted)))

knn_clf = KNeighborsClassifier()
knn_clf.fit(X_train, y_train)
predicted = knn_clf.predict(X_test)
print("KNNの正解率 = " + str(accuracy_score(y_test, predicted)))
print("KNNのF値 = " + str(f1_score(y_test, predicted)))

clf = svm.SVC()
clf.fit(X_train, y_train)
predicted = clf.predict(X_test)
print("SVCの正解率 = " + str(accuracy_score(y_test, predicted)))
print("SVCのF値 = " + str(f1_score(y_test, predicted)))
